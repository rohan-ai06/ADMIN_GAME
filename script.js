...
function resizeCanvas() {
    ...
    if (ship.x > canvas.width) ship.x = canvas.width - 100;
    if (ship.y > canvas.height) ship.y = canvas.height - 100;
}
...
const bossVoiceLines = {
    ...
    wrongEasy: [
        "Ha! You failed basic tier?",
        "My t2.micro is smarter than you.",
        "Latency detected in your brain."
    ],
    wrongHard: [
        "That architecture was flawed.",
        "Scaling down your life expectancy.",
        "You lack the permissions for that.",
        "Deployment failed."
    ],
    streak: [
...
function speakBoss(category) {
    ...
    // Cancel previous speech & reset volume safety
    window.speechSynthesis.cancel();
    if (sounds.bgMusic) sounds.bgMusic.volume = 0.7;

    const utterance = new SpeechSynthesisUtterance(text);
    utterance.pitch = 2.0; // MAX Pitch (Cyber-Psycho)
    ...
    // Try to find a robotic voice (Avoid "Google" which sounds too human)
    const voices = window.speechSynthesis.getVoices();
    // Prioritize Microsoft Zira/David or generic system voices
    const roboticVoice = voices.find(v => v.name.includes("Zira") || v.name.includes("David") || v.name.includes("Desktop"));

    if (roboticVoice) {
        utterance.voice = roboticVoice;
    } else {
        // Fallback: If we only have Google, use it but maybe tweak rate more
...
function triggerBossAttack() {
    ...
    const leftArmorX = boss.x - 220 * 1.3;
    const rightArmorX = boss.x + 220 * 1.3;
    const armorY = boss.y + 20 * 1.3;
    const coreX = boss.x;
    const coreY = boss.y + 45 * 1.3;
    ...
    setTimeout(() => {
        ...
        for (let b = 0; b < 3; b++) {
            ...
            const endX = ship.x + (b - 1) * 300;

            for (let i = 0; i < 150; i++) {
                const t = i / 150;
                particles.push({
                    x: coreX + (endX - coreX) * t + (Math.random() - 0.5) * 60, // Wide beam
                    y: coreY + (ship.y - coreY) * t + (Math.random() - 0.5) * 60,
...
function drawBoss() {
    ...
    ctx.scale(1.3, 1.3); // Increased size
...
function drawScorchMarks() {
    boss.scorchMarks.forEach(mark => {
        ...
        ctx.beginPath();
        ctx.arc(0, 0, mark.radius * 0.6, 0, Math.PI * 2);
        ctx.fill();
        ctx.restore();
    });
}
    ...
    boss.scorchMarks.forEach(mark => {
        ...
        if (Math.random() < 0.4) {
            particles.push(createParticle(
                absX + (Math.random() - 0.5) * mark.radius,
                absY + (Math.random() - 0.5) * mark.radius,
                '#444444', 0.8
            ));
        }
        // Increased Fire
...
function drawTargets() {
    targets.forEach(t => {
        ...
        const size = 30;
        const color = t.locked ? '#00ff00' : '#ff0000';
...
function updateTargets() {
    targets = targets.filter(t => {
        t.life -= 0.02;
        if (t.life < 0.8) t.locked = true; // Lock after short delay
        return t.life > 0;
    });
}
...
function fireRockets() {
    ...
    for (let i = 0; i < rocketCount; i++) {
        ...
        const targetX = boss.x + pt.x + (Math.random() - 0.5) * 40;
        const targetY = boss.y + pt.y + (Math.random() - 0.5) * 40;
        ...
        targets.push({
            x: targetX,
            y: targetY,
            life: 3.0, // Increased life for longer visibility
            locked: false,
...
function checkCollisions() {
    ...
    rockets.forEach((rocket, idx) => {
        ...
        if (rocket.target) {
            ...
            if (Math.abs(dx) < 390 && Math.abs(dy) < 160) hit = true;
        }
    ...
    if (laserFiring) {
        const leftWingX = ship.x - 54 * 1.5; // Updated scale
        const rightWingX = ship.x + 54 * 1.5;
        const targetY = boss.y + 40;

        // Increased hitbox for bigger boss
        if (Math.abs(boss.x - leftWingX) < 325 || Math.abs(boss.x - rightWingX) < 325) {
...
function gameLoop() {
    ...
    // Projectiles
    drawTargets(); // Draw targets under rockets
    drawRockets();
    updateRockets();
    updateTargets(); // Update target timers
    drawLaser();
    drawPlasmaWaves();
    updatePlasmaWaves();
